...
public class maximusMazeTemplate extends PApplet {
...
public void drawMaze(){
  if(pieces.size() > 0)
  {
    for(int j=0; j<rows; j++){
      for(int i=0; i<cols; i++){
        pieces.get( PApplet.parseInt(i +j*cols) ).display();
      }
    }
  }
}
public PVector findDir(int n){
  if(n == 0){
...
class mazePiece{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "maximusMazeTemplate" };
    if (passedArgs != null) {
